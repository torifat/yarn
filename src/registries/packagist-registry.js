...
import PackagistResolver from '../resolvers/registries/packagist-resolver.js';
...
const DEFAULT_REGISTRY = 'https://packagist.org/packages';
...
export default class PackagistRegistry extends Registry {
  ...
  async checkOutdated(config: Config, name: string, range: string): CheckOutdatedReturn {
    ...
    // const req = await this.request(PackagistRegistry.escapeName(name));
    // if (!req) {
    ...
    //   wanted: (await PackagistResolver.findVersionInRegistryResponse(config, range, req)).version,
    // };
  }

  async getPossibleConfigLocations(filename: string): Promise<Array<[boolean, string, string]>> {
    console.log('getPossibleConfigLocations');
    // const possibles = [
